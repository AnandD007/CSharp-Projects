...
{
    internal class ReplaceFileContents
    {
        internal void ReplaceFileContentFunction()
        {
            try
            {
                string textRead;
                ...
                if (fileFoundStatus == true)
                {
                    ...
                    string[] lines = File.ReadAllLines(userFullFileName);
                    ...
                    {
                        lines[i] = lines[i].Replace(wordToReplace, replacementWord);
                    }

                    DirectoryInfo ReaderFilePath = new DirectoryInfo(@"" + fileReadLocation);
                    if (ReaderFilePath == null || !(ReaderFilePath.Exists))
                    {
                        throw new DirectoryNotFoundException();
                    }
                    else
                    {
                        string ReadFilePath = fileReadLocation + "\\" + userReadFileName;
                        File.WriteAllLines(ReadFilePath, lines);
                    }
